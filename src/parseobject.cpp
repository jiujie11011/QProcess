...
void ParseObject::getQueuedXml()
{
  ...
  QString currentCodecName_;

  {
    QMutexLocker locker(&mutex_);
    if (idsQueue_.count()) {
      feedId = idsQueue_.dequeue();
      currentXml_ = xmlsQueue_.dequeue();
      currentDtReady_ = dtReadyQueue_.dequeue();
      currentCodecName_ = codecNameQueue_.dequeue();
      qDebug() << "xmlsQueue_ >>" << feedId << "count=" << xmlsQueue_.count();
    }
  }

  if (!currentXml_.isNull()) {
...
{
  if (mainApp->isSaveDataLastFeed() && !mainApp->isNoDebugOutput()) {
  ...
  if (q.first()) {
    ...
    fetchType_ = q.value(5).toInt();
    fetchRule_ = q.value(6).toString();
    fetchScript_ = q.value(7).toString();
  }
  ...
  if (!doc.setContent(convertData, false, &errorStr, &errorLine, &errorColumn)) {
    ...
    } else if (fetchType_ == 2) {
      // Script mode: run the user script, parse its stdout as RSS/Atom.
      parseScript(convertData, feedUrl);
    } else if (fetchType_ == 1) {
      // XPath mode: the fetched payload is HTML/XML, not a feed document.
      parseXPath(convertData, feedUrl);
    }
  ...
  {
    StatisticsService stats(Database::connection("secondConnection"));
    stats.addEvent(StatType::FeedRefresh);
  }
...
void ParseObject::addAtomNewsIntoBase(NewsItemStruct *newsItem)
{
  ...
  if (!newsItem->id.isEmpty()) {         // search by guid if present
    const QList<int> &candidates = guidIndex_.value(newsItem->id);
    ...
    if (!newsItem->updated.isEmpty()) {    // search by pubDate if present
      if (!publishedIndex_.value(newsItem->updated).isEmpty())
        isDuplicate = true;
    } else {                              // ... or by title
      if (!newsItem->title.isEmpty() &&
          !titleIndex_.value(newsItem->title).isEmpty())
        isDuplicate = true;
    }
  }
  ...
  if (!isDuplicate && !isOld) {
    ...
    if (mainApp->mainWindow()->markIdenticalNewsRead_) {
      q.prepare("SELECT id FROM news WHERE title = :title COLLATE NOCASE AND feedId != :id");
...
// ----------------------------------------------------------------------------
bool ParseObject::parseScript(const QString &data, const QString &feedUrl)
{
  if (fetchScript_.isEmpty()) {
    qWarning() << "Script feed has empty fetchScript, feedId =" << parseFeedId_;
    return false;
  }

  ScriptFeedRunner runner;
  QByteArray xmlData = runner.run(fetchScript_, data.toUtf8());
  if (xmlData.isEmpty()) {
    qWarning() << "Script feed error, feedId =" << parseFeedId_
               << "error:" << runner.lastError();
    return false;
  }

  // Decode to QString (script output is expected to be RSS/Atom XML).
  QString xmlStr;
  QTextCodec *codec = QTextCodec::codecForUtfText(xmlData, QTextCodec::codecForName("UTF-8"));
  if (codec)
    xmlStr = codec->toUnicode(xmlData);
  else
    xmlStr = QString::fromUtf8(xmlData);

  QDomDocument doc;
  QString errorStr;
  int errorLine;
  int errorColumn;
  if (!doc.setContent(xmlStr, false, &errorStr, &errorLine, &errorColumn)) {
    qWarning() << QString("Script parse error: url %1, id %2, line %3, column %4: %5").
                  arg(feedUrl).arg(parseFeedId_).
                  arg(errorLine).arg(errorColumn).arg(errorStr);
    return false;
  }
  ...
  QDomElement rootElem = doc.documentElement();
  QString feedType = rootElem.tagName();
  qDebug() << "Script feed type: " << feedType;
  ...
  if (feedType == "feed") {
    ...
    qWarning() << "Script output is not RSS/Atom, feedId =" << parseFeedId_;
    return false;
  }
  return true;
}
...
// ----------------------------------------------------------------------------
void ParseObject::parseXPath(const QString &data, const QString &feedUrl)
{
  Q_UNUSED(feedUrl)

  if (fetchRule_.isEmpty()) {
    qWarning() << "XPath feed has empty fetchRule, feedId =" << parseFeedId_;
    return;
  }

  XPathFeedParser parser;
  QList<XPathNewsItem> items = parser.parse(data, fetchRule_);

  qDebug() << "XPath parse: feedId =" << parseFeedId_
           << "items =" << items.count();

  foreach (const XPathNewsItem &xitem, items) {
    NewsItemStruct newsItem;
    newsItem.id = xitem.link;                 // use link as guid
    newsItem.title = xitem.title;
    newsItem.link = xitem.link;
    newsItem.updated = xitem.date;
    newsItem.author = xitem.author;
    if (xitem.description.isEmpty()) {
      newsItem.description = xitem.title;
      newsItem.content = xitem.description;
    } else {
      newsItem.description = xitem.description;
      newsItem.content = xitem.description;
    }
    addRssNewsIntoBase(&newsItem);
  }
...
void ParseObject::addRssNewsIntoBase(NewsItemStruct *newsItem)
{
  ...
  bool isDuplicate = false;
  if (!newsItem->id.isEmpty()) {         // search by guid if present
    const QList<int> &guidCandidates = guidIndex_.value(newsItem->id);
    foreach (int i, guidCandidates) {
      if (!newsItem->updated.isEmpty()) {  // search by pubDate if present
    ...
    if (!isDuplicate) {
      if (!newsItem->updated.isEmpty()) {
        const QList<int> &pubCandidates = publishedIndex_.value(newsItem->updated);
        foreach (int i, pubCandidates) {
    ...
    const QList<int> &linkCandidates = linkIndex_.value(newsItem->link);
    foreach (int i, linkCandidates) {
      if (!newsItem->updated.isEmpty()) {  // search by pubDate if present
        if (!duplicateNewsMode_) {
          if (publishedList_.at(i) == newsItem->updated)
            isDuplicate = true;
...
void ParseObject::runUserFilter(int feedId, int filterId)
{
  ...
  if (filterId != -1) {
    isAllFilters = false;
    q.prepare("SELECT enable, type FROM filters WHERE id = ? AND feeds LIKE '%,?,%'");
    q.addBindValue(filterId);
    q.addBindValue(feedId);
    q.exec();
  } else {
    q.prepare("SELECT enable, type, id FROM filters WHERE feeds LIKE '%,?,%' ORDER BY num");
    q.addBindValue(feedId);
    q.exec();
  }
  ...
  while (q.next()) {
    ...
    while (q1.next()) {
      ...
      case 5: // action -> Show News in Notifier
        ...
        break;
      case 6: // action -> Mark news as unread
        if (!qStr1.isNull()) qStr1.append(",");
        qStr1.append(" new=1, read=0");
        break;
      case 7: // action -> Move to category
        if (!qStr1.isNull()) qStr1.append(",");
        qStr1.append(QString(" category='%1'").
                     arg(q1.value(1).toString().replace("'", "''")));
        break;
      case 8: // action -> Save AI summary
        if (!qStr1.isNull()) qStr1.append(",");
        qStr1.append(" aiSummary=1");
        break;
      }
    }
    ...
    if ((filterType == 1) || (filterType == 2)) {
      ...
      while (q1.next()) {
        ...
        case 6: // field -> News
          ...
          break;
        case 7: // field -> Published
          switch (q1.value(1).toInt()) {
          case 0: // condition -> is
            qStr1.append(QString("published == '%1' ").arg(content));
            break;
          case 1: // condition -> isn't
            qStr1.append(QString("published != '%1' ").arg(content));
            break;
          case 2: // condition -> is before
            qStr1.append(QString("published < '%1' ").arg(content));
            break;
          case 3: // condition -> is after
            qStr1.append(QString("published > '%1' ").arg(content));
            break;
          }
          break;
                                   ...
                                   const QString &updated, const QString &lastBuildDate)
{
  ...
  int feedParId = 0;
  q.prepare("SELECT parentId, htmlUrl, title FROM feeds WHERE id = ?");
  q.addBindValue(feedId);
  q.exec();
  if (q.first()) {
  ...
  q.prepare("SELECT count(id) FROM news WHERE feedId = ? AND deleted = 0");
  q.addBindValue(feedId);
  q.exec();
  if (q.first()) undeleteCount = q.value(0).toInt();

  // Count unread news
  q.prepare("SELECT count(read) FROM news WHERE feedId = ? AND read = 0 AND deleted = 0");
  q.addBindValue(feedId);
  q.exec();
  if (q.first()) unreadCount = q.value(0).toInt();

  // Count new news
  q.prepare("SELECT count(new) FROM news WHERE feedId = ? AND new = 1 AND deleted = 0");
  q.addBindValue(feedId);
  q.exec();
  ...
  q.prepare("SELECT unread, newCount, undeleteCount FROM feeds WHERE id = ?");
  q.addBindValue(feedId);
  q.exec();
  if (q.first()) {
  ...
  q.prepare("UPDATE feeds SET unread = ?, newCount = ?, undeleteCount = ? WHERE id = ?");
  q.addBindValue(unreadCount);
  q.addBindValue(newNewsCount);
  ...
  while (l_feedParId) {
    ...
    q.prepare("SELECT sum(unread), sum(newCount), sum(undeleteCount), "
              "max(updated) FROM feeds WHERE parentId = ?");
