  ...
  , updatePaused_(false)
{
  ...
  progressService_ = new ProgressService(this);
  statisticsService_ = new StatisticsService(db_);
  aiAssistant_ = new AIAssistant(this);
  translationService_ = new TranslationService(aiAssistant_, this);
  intelligentRefresh_ = new IntelligentRefreshCalculator(db_);
...
void MainWindow::createStatusBar()
{
  ...
  statusUpdating_ = new QLabel(this);
  statusUpdating_->hide();
  statusBar()->addPermanentWidget(statusUpdating_);
...
void MainWindow::createActions()
{
  ...
  importFeedsJsonAct_ = new QAction(this);
  importFeedsJsonAct_->setObjectName("importFeedsJsonAct");
  importFeedsJsonAct_->setIcon(QIcon(":/images/importFeeds"));
  this->addAction(importFeedsJsonAct_);
  connect(importFeedsJsonAct_, SIGNAL(triggered()), this, SLOT(slotImportFeedsJson()));

  exportFeedsJsonAct_ = new QAction(this);
  exportFeedsJsonAct_->setObjectName("exportFeedsJsonAct");
  exportFeedsJsonAct_->setIcon(QIcon(":/images/exportFeeds"));
  this->addAction(exportFeedsJsonAct_);
  connect(exportFeedsJsonAct_, SIGNAL(triggered()), this, SLOT(slotExportFeedsJson()));

  feedsManagementAct_ = new QAction(this);
  feedsManagementAct_->setObjectName("feedsManagementAct");
  feedsManagementAct_->setIcon(QIcon(":/images/exportFeeds"));
  this->addAction(feedsManagementAct_);
  connect(feedsManagementAct_, SIGNAL(triggered()), this, SLOT(slotFeedsManagement()));

  undoLastMarkAct_ = new QAction(this);
  undoLastMarkAct_->setObjectName("undoLastMarkAct");
  undoLastMarkAct_->setIcon(QIcon(":/images/markRead"));
  this->addAction(undoLastMarkAct_);
  connect(undoLastMarkAct_, SIGNAL(triggered()), this, SLOT(slotUndoLastMark()));

  statisticsAct_ = new QAction(this);
  statisticsAct_->setObjectName("statisticsAct");
  statisticsAct_->setIcon(QIcon(":/images/markReadAll"));
  this->addAction(statisticsAct_);
  connect(statisticsAct_, SIGNAL(triggered()), this, SLOT(slotShowStatistics()));

  aiAssistantAct_ = new QAction(this);
  aiAssistantAct_->setObjectName("aiAssistantAct");
  aiAssistantAct_->setIcon(QIcon(":/images/markRead"));
  this->addAction(aiAssistantAct_);
  connect(aiAssistantAct_, SIGNAL(triggered()), this, SLOT(slotShowAIDialog()));

  saveProgressAct_ = new QAction(this);
  saveProgressAct_->setObjectName("saveProgressAct");
  this->addAction(saveProgressAct_);
  connect(saveProgressAct_, SIGNAL(triggered()), this, SLOT(slotSaveProgress()));
...
void MainWindow::createShortcut()
{
  ...
  listActions_.append(importFeedsJsonAct_);
  listActions_.append(exportFeedsJsonAct_);
  listActions_.append(feedsManagementAct_);
  listActions_.append(undoLastMarkAct_);
  listActions_.append(statisticsAct_);
  listActions_.append(createBackupAct_);
...
void MainWindow::createMenu()
{
  ...
  fileMenu_->addAction(importFeedsJsonAct_);
  fileMenu_->addAction(exportFeedsJsonAct_);
  fileMenu_->addAction(feedsManagementAct_);
  fileMenu_->addAction(undoLastMarkAct_);
  fileMenu_->addAction(statisticsAct_);
...
void MainWindow::loadSettings()
{
  ...
  QString browserStandardFont = settings.value(
        "browserStandardFont", QWebEngineProfile::defaultProfile()->settings()->fontFamily(QWebEngineSettings::StandardFont)).toString();
  QString browserFixedFont = settings.value(
        "browserFixedFont", QWebEngineProfile::defaultProfile()->settings()->fontFamily(QWebEngineSettings::FixedFont)).toString();
  QString browserSerifFont = settings.value(
        "browserSerifFont", QWebEngineProfile::defaultProfile()->settings()->fontFamily(QWebEngineSettings::SerifFont)).toString();
  QString browserSansSerifFont = settings.value(
        "browserSansSerifFont", QWebEngineProfile::defaultProfile()->settings()->fontFamily(QWebEngineSettings::SansSerifFont)).toString();
  QString browserCursiveFont = settings.value(
        "browserCursiveFont", QWebEngineProfile::defaultProfile()->settings()->fontFamily(QWebEngineSettings::CursiveFont)).toString();
  QString browserFantasyFont = settings.value(
        "browserFantasyFont", QWebEngineProfile::defaultProfile()->settings()->fontFamily(QWebEngineSettings::FantasyFont)).toString();
  int browserDefaultFontSize = settings.value(
        "browserDefaultFontSize", QWebEngineProfile::defaultProfile()->settings()->fontSize(QWebEngineSettings::DefaultFontSize)).toInt();
  int browserFixedFontSize = settings.value(
        "browserFixedFontSize", QWebEngineProfile::defaultProfile()->settings()->fontSize(QWebEngineSettings::DefaultFixedFontSize)).toInt();
  int browserMinFontSize = settings.value(
        "browserMinFontSize", QWebEngineProfile::defaultProfile()->settings()->fontSize(QWebEngineSettings::MinimumFontSize)).toInt();
  int browserMinLogFontSize = settings.value(
        "browserMinLogFontSize", QWebEngineProfile::defaultProfile()->settings()->fontSize(QWebEngineSettings::MinimumLogicalFontSize)).toInt();
  ...
  excludedFeeds_ = settings.value("Interaction/excludedFeeds", QStringList()).toStringList();

  cleanupEnabled_ = settings.value("Cleanup/enabled", false).toBool();
  cleanupStrategy_ = settings.value("Cleanup/strategy", "time").toString();
  ...
  aiApiKey_ = settings.value("AI/apiKey").toString();
  aiBaseUrl_ = settings.value(
        "AI/baseUrl", "https://api.openai.com/v1/chat/completions").toString();
  aiSummaryLength_ = settings.value("AI/summaryLength", 200).toInt();
  aiHistoryRetention_ = settings.value("AI/historyRetention", 50).toInt();
  aiOfflineCache_ = settings.value("AI/offlineCache", false).toBool();
  aiAutoTranslate_ = settings.value("AI/autoTranslate", false).toBool();
  aiTranslateLang_ = settings.value("AI/translateLang", "zh").toString();
  aiTranslationEngine_ = settings.value("AI/translationEngine", "google").toString();
  ...
  // WebEngine: no direct equivalent for setMaximumPagesInCache
  QWebEngineProfile::defaultProfile()->settings()->setAttribute(
        QWebEngineSettings::ErrorPageEnabled, false);
  // WebEngine: offline storage handled by profile settings
...
void MainWindow::saveSettings()
{
  ...
  settings.setValue("Interaction/hoverMarkRead", hoverMarkRead_);
  settings.setValue("Interaction/hoverMarkDelay", hoverMarkDelay_);
  settings.setValue("Interaction/scrollMarkRead", scrollMarkRead_);
  settings.setValue("Interaction/viewportMarkRead", viewportMarkRead_);
  settings.setValue("Interaction/markExcludeOnlyStarred", markExcludeOnlyStarred_);
  settings.setValue("Interaction/excludedGroups", excludedGroups_);
  settings.setValue("Interaction/excludedFeeds", excludedFeeds_);

  settings.setValue("Cleanup/enabled", cleanupEnabled_);
  settings.setValue("Cleanup/strategy", cleanupStrategy_);
  ...
  settings.setValue("AI/apiKey", aiApiKey_);
  settings.setValue("AI/baseUrl", aiBaseUrl_);
  settings.setValue("AI/summaryLength", aiSummaryLength_);
  settings.setValue("AI/historyRetention", aiHistoryRetention_);
  settings.setValue("AI/offlineCache", aiOfflineCache_);
  settings.setValue("AI/autoTranslate", aiAutoTranslate_);
  settings.setValue("AI/translateLang", aiTranslateLang_);
  settings.setValue("AI/translationEngine", aiTranslationEngine_);
  settings.setValue("AI/deeplKey", aiDeeplKey_);
  settings.setValue("AI/baiduAppId", aiBaiduAppId_);
  settings.setValue("AI/baiduKey", aiBaiduKey_);
  settings.setValue("AI/autoSummary", aiAutoSummary_);
  settings.setValue("AI/autoRecommend", aiAutoRecommend_);
  settings.setValue("AI/maxTokens", aiMaxTokens_);
...
// ----------------------------------------------------------------------------
void MainWindow::slotExportFeedsJson()
{
  QString fileName = QFileDialog::getSaveFileName(this, tr("Export Feeds to JSON"),
                                                  QDir::homePath(),
                                                  tr("JSON-Files (*.json)"));
  ...
  QString jsonData = exportFeedsToJson(feedsModel_, db_);
  if (jsonData.isEmpty()) {
    statusBar()->showMessage(tr("Export: no feeds to export"), 3000);
    return;
  }
  ...
  file.close();
  statusBar()->showMessage(tr("Feeds exported to JSON"), 3000);
}

// ----------------------------------------------------------------------------
void MainWindow::slotImportFeedsJson()
{
  QString fileName = QFileDialog::getOpenFileName(this, tr("Import Feeds from JSON"),
                                                  QDir::homePath(),
                                                  tr("JSON-Files (*.json)"));
  ...
  int addedCount = importFeedsFromJson(jsonData, db_);
  if (addedCount < 0) {
    statusBar()->showMessage(tr("Import: invalid JSON file"), 3000);
    return;
  }

  feedsModel_->refresh();
  statusBar()->showMessage(tr("Imported %1 feeds").arg(addedCount), 3000);
}

// ----------------------------------------------------------------------------
void MainWindow::slotFeedsManagement()
{
  FeedsManagementDialog dialog(this, db_);
  dialog.exec();
  if (!dialog.deletedFeedIds().isEmpty()) {
    feedsModel_->refresh();
...
void MainWindow::slotUndoLastMark()
{
  ...
  if (currentNewsTab->interactiveMarkController_ &&
      currentNewsTab->interactiveMarkController_->undoLastBulkMark()) {
    setNewsFilter(newsFilterGroup_->checkedAction(), false);
    recountCategoryCounts();
    if (progressService_) progressService_->flush();
  }
}

// ----------------------------------------------------------------------------
void MainWindow::slotShowStatistics()
{
  StatisticsDialog dialog(this, db_);
  dialog.exec();
}

// ----------------------------------------------------------------------------
void MainWindow::slotShowAIDialog()
{
  ...
  if (currentNewsTab && (currentNewsTab->type_ < NewsTabWidget::TabTypeWeb)) {
    ...
    if (curIndex.isValid()) {
      ...
      context.newsId = model->dataField(curIndex.row(), "id").toInt();
      context.title = model->dataField(curIndex.row(), "title").toString();
      context.content = model->dataField(curIndex.row(), "content").toString();
      if (context.content.isEmpty())
        context.content = model->dataField(curIndex.row(), "description").toString();
      context.category = model->dataField(curIndex.row(), "category").toString();
    }
  }

  AIDialog dialog(this, aiAssistant_);
  dialog.setArticleContext(context);
  dialog.exec();
}
...
// ----------------------------------------------------------------------------
void MainWindow::slotCheckDiskSpace()
{
#ifdef HAVE_QT5
  QStorageInfo storage(mainApp->dataDir());
  if (!storage.isValid() || storage.isReady() == false)
    return;
  ...
  QMessageBox::StandardButton button = QMessageBox::warning(
        ...
        tr("Available disk space is less than 100 MB (%1 MB). "
           "Old news cleanup is recommended.").arg(availableMB),
...
void MainWindow::slotUpdateFeed(int feedId, bool changed, int newCount, bool finish)
{
  ...
  // Intelligent refresh: recompute this feed's auto-update interval from its
  // recent publication frequency (see intelligentrefreshcalculator).
  updateIntelligentRefreshInterval(feedId);
...
void MainWindow::slotFeedSelected(QModelIndex index, bool createTab)
{
  ...
  if ((newsRow >= 0) && (openingFeedAction_ == 0)) {
    QModelIndex idIndex = newsModel_->index(newsRow, newsModel_->fieldIndex("id"));
    int newsId = newsModel_->data(idIndex, Qt::EditRole).toInt();
    QSqlQuery q(db_);
    q.prepare("SELECT scrollPos FROM news WHERE id = ?");
    q.addBindValue(newsId);
    q.exec();
    if (q.first()) {
      int scrollPos = q.value(0).toInt();
      if (scrollPos > 0)
        newsView_->verticalScrollBar()->setValue(scrollPos);
    }
    if (progressService_)
      progressService_->setContext(feedId, newsId,
...
void MainWindow::showOptionDlg(int index)
{
  ...
  optionsDialog_->hoverMarkRead_->setChecked(hoverMarkRead_);
  for (int i = 0; i < optionsDialog_->hoverMarkDelay_->count(); i++) {
    if (optionsDialog_->hoverMarkDelay_->itemData(i).toInt() == hoverMarkDelay_) {
      optionsDialog_->hoverMarkDelay_->setCurrentIndex(i);
      break;
    }
  }
  optionsDialog_->scrollMarkRead_->setChecked(scrollMarkRead_);
  optionsDialog_->viewportMarkRead_->setChecked(viewportMarkRead_);
  optionsDialog_->markExcludeOnlyStarred_->setChecked(markExcludeOnlyStarred_);
  optionsDialog_->excludedGroups_->setText(excludedGroups_.join(","));
  optionsDialog_->excludedFeeds_->setText(excludedFeeds_.join(","));

  optionsDialog_->cleanupEnabled_->setChecked(cleanupEnabled_);
  optionsDialog_->cleanupStrategyTime_->setChecked(cleanupStrategy_ == "time");
  optionsDialog_->cleanupStrategyCount_->setChecked(cleanupStrategy_ == "count");
  ...
  optionsDialog_->aiBaseUrl_->setText(aiBaseUrl_);
  int aiModelIndex = optionsDialog_->aiModel_->findText(aiModel_);
  if (aiModelIndex >= 0) optionsDialog_->aiModel_->setCurrentIndex(aiModelIndex);
  else optionsDialog_->aiModel_->setEditText(aiModel_);
  optionsDialog_->aiSummaryLength_->setValue(aiSummaryLength_);
  optionsDialog_->aiMaxTokens_->setValue(aiMaxTokens_);
  for (int i = 0; i < optionsDialog_->aiHistoryRetention_->count(); i++) {
    if (optionsDialog_->aiHistoryRetention_->itemData(i).toInt() == aiHistoryRetention_) {
      optionsDialog_->aiHistoryRetention_->setCurrentIndex(i);
      break;
    }
  }
  optionsDialog_->aiOfflineCache_->setChecked(aiOfflineCache_);
  optionsDialog_->aiAutoTranslate_->setChecked(aiAutoTranslate_);
  optionsDialog_->aiAutoTranslateToggled(aiAutoTranslate_);
  int aiLangIndex = optionsDialog_->aiTranslateLang_->findData(aiTranslateLang_);
  if (aiLangIndex >= 0) optionsDialog_->aiTranslateLang_->setCurrentIndex(aiLangIndex);
  else optionsDialog_->aiTranslateLang_->setEditText(aiTranslateLang_);

  int engineIndex = optionsDialog_->aiTranslationEngine_->findData(aiTranslationEngine_);
  if (engineIndex >= 0) {
    optionsDialog_->aiTranslationEngine_->setCurrentIndex(engineIndex);
    optionsDialog_->slotTranslationEngineChanged(engineIndex);
  }
  optionsDialog_->aiDeeplKey_->setText(aiDeeplKey_);
  optionsDialog_->aiBaiduAppId_->setText(aiBaiduAppId_);
  optionsDialog_->aiBaiduKey_->setText(aiBaiduKey_);

  optionsDialog_->aiAutoSummary_->setChecked(aiAutoSummary_);
  optionsDialog_->aiAutoRecommend_->setChecked(aiAutoRecommend_);
  ...
  hoverMarkRead_ = optionsDialog_->hoverMarkRead_->isChecked();
  hoverMarkDelay_ = optionsDialog_->hoverMarkDelay_->currentData().toInt();
  ...
  for (int i = 0; i < excludedFeeds_.count(); i++)
    excludedFeeds_[i] = excludedFeeds_.at(i).trimmed();

  cleanupEnabled_ = optionsDialog_->cleanupEnabled_->isChecked();
  cleanupStrategy_ = optionsDialog_->cleanupStrategyCount_->isChecked() ? "count" : "time";
  ...
  aiBaseUrl_ = optionsDialog_->aiBaseUrl_->text().trimmed();
  if (aiBaseUrl_.isEmpty())
    aiBaseUrl_ = "https://api.openai.com/v1/chat/completions";
  if (optionsDialog_->aiModel_->currentIndex() >= 0)
    aiModel_ = optionsDialog_->aiModel_->currentText();
  else
    aiModel_ = optionsDialog_->aiModel_->lineEdit()->text();
  aiSummaryLength_ = optionsDialog_->aiSummaryLength_->value();
  aiHistoryRetention_ = optionsDialog_->aiHistoryRetention_->currentData().toInt();
  aiOfflineCache_ = optionsDialog_->aiOfflineCache_->isChecked();
  aiAutoTranslate_ = optionsDialog_->aiAutoTranslate_->isChecked();
  if (optionsDialog_->aiTranslateLang_->currentIndex() >= 0)
    aiTranslateLang_ = optionsDialog_->aiTranslateLang_->currentData().toString();
  else
    aiTranslateLang_ = optionsDialog_->aiTranslateLang_->currentText();
  if (optionsDialog_->aiTranslationEngine_->currentIndex() >= 0)
    aiTranslationEngine_ = optionsDialog_->aiTranslationEngine_->currentData().toString();
  aiDeeplKey_ = optionsDialog_->aiDeeplKey_->text().trimmed();
  aiBaiduAppId_ = optionsDialog_->aiBaiduAppId_->text().trimmed();
  aiBaiduKey_ = optionsDialog_->aiBaiduKey_->text().trimmed();
  aiAutoSummary_ = optionsDialog_->aiAutoSummary_->isChecked();
  aiAutoRecommend_ = optionsDialog_->aiAutoRecommend_->isChecked();
  aiMaxTokens_ = optionsDialog_->aiMaxTokens_->value();
...
// ----------------------------------------------------------------------------
void MainWindow::updateIntelligentRefreshInterval(int feedId)
{
  // Only adjust feeds with per-feed auto-update enabled (custom interval).
  if (!updateFeedsIntervalSec_.contains(feedId))
    return;

  int intervalSec = intelligentRefresh_->calculateInterval(feedId);
  updateFeedsIntervalSec_.insert(feedId, intervalSec);
  // Reset the countdown so the new interval takes effect from now.
...
void MainWindow::retranslateStrings()
{
  ...
  importFeedsJsonAct_->setText(tr("Import Feeds from &JSON..."));
  importFeedsJsonAct_->setToolTip(tr("Import Feeds from JSON File"));

  exportFeedsJsonAct_->setText(tr("Export Feeds to &JSON..."));
  exportFeedsJsonAct_->setToolTip(tr("Export Feeds to JSON File"));

  feedsManagementAct_->setText(tr("Feeds &Management..."));
  feedsManagementAct_->setToolTip(tr("Manage feeds and groups"));

  undoLastMarkAct_->setText(tr("Undo Last Auto &Mark"));
  undoLastMarkAct_->setToolTip(tr("Undo last interactive mark as read"));

  statisticsAct_->setText(tr("&Statistics..."));
  statisticsAct_->setToolTip(tr("Show reading statistics"));

  aiAssistantAct_->setText(tr("&AI Assistant..."));
  aiAssistantAct_->setToolTip(tr("Open AI chat assistant"));

  saveProgressAct_->setText(tr("Save Progress Now"));
...
void MainWindow::showFeedPropertiesDlg()
{
  ...
  properties.proxy.on = feedsModel_->dataField(index, "proxyEnabled").toBool();
  properties.proxy.url = feedsModel_->dataField(index, "proxyURL").toString();
  ...
  q.prepare("UPDATE feeds SET text = ?, xmlUrl = ?, displayOnStartup = ?, "
            ...
            "javaScriptEnable = ?, proxyEnabled = ?, proxyURL = ? WHERE id == ?");
  ...
  q.addBindValue(properties.proxy.on ? 1 : 0);
  q.addBindValue(properties.proxy.url);
  ...
  QModelIndex indexProxyEnabled = feedsModel_->indexSibling(index, "proxyEnabled");
  QModelIndex indexProxyURL = feedsModel_->indexSibling(index, "proxyURL");
  ...
  feedsModel_->setData(indexProxyEnabled, properties.proxy.on ? 1 : 0);
  feedsModel_->setData(indexProxyURL, properties.proxy.url);
 ...
 *---------------------------------------------------------------------------*/
void MainWindow::slotPrintPreview(QWebEnginePage *page)
{
  if (currentNewsTab->type_ == NewsTabWidget::TabTypeDownloads) return;
...
void MainWindow::slotSavePageAs()
{
  ...
  // QWebEnginePage::toHtml() is asynchronous - do the file I/O in the callback
  const QFileInfo fileInfo(fileName);
  const QString titleString = title;
  QPointer<MainWindow> guard(this);

  currentNewsTab->webView_->page()->toHtml([guard, fileName, fileInfo, titleString](const QString &html) {
    if (!guard) return;

    QFile file(fileName);
    if (!file.open(QIODevice::WriteOnly)) {
      guard->statusBar()->showMessage(MainWindow::tr("Save As: can't open a file"), 3000);
      return;
    }
    if (fileInfo.suffix() == "txt") {
      QTextDocumentFragment textDocument = QTextDocumentFragment::fromHtml(html);
      ...
      file.write(codec->fromUnicode(htmlOut));
    }
    file.close();
  });
}
