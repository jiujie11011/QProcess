...
QWizardPage *AddFeedWizard::createUrlFeedPage()
{
  ...
  fetchTypeCombo_ = new QComboBox(this);
  fetchTypeCombo_->addItem(tr("RSS/Atom feed"), 0);
  fetchTypeCombo_->addItem(tr("XPath scraping"), 1);
  fetchTypeCombo_->addItem(tr("Custom script"), 2);
  connect(fetchTypeCombo_, SIGNAL(currentIndexChanged(int)),
          this, SLOT(slotFetchTypeChanged(int)));

  xpathHintLabel_ = new QLabel(this);
  xpathHintLabel_->setWordWrap(true);
  xpathHintLabel_->setText(tr(
    "XPath rules (JSON):\n"
    "{\"item\":\"//div[contains(@class,'item')]\",\n"
    " \"title\":\".//h2/a\", \"link\":\".//h2/a/@href\",\n"
    " \"description\":\".//div[contains(@class,'desc')]\"}"));

  xpathRuleEdit_ = new QPlainTextEdit(this);
  xpathRuleEdit_->setPlainText(
    "{\n  \"item\": \"//div[contains(@class,'item')]\",\n"
    "  \"title\": \".//h2/a\",\n  \"link\": \".//h2/a/@href\",\n"
    "  \"description\": \".//div[contains(@class,'desc')]\"\n}");
  xpathRuleEdit_->setVisible(false);
  xpathHintLabel_->setVisible(false);

  scriptHintLabel_ = new QLabel(tr(
    "Path to a script (.py/.sh/.js/.rb) that reads the page on stdin and "
    "writes RSS/Atom XML to stdout."), this);
  scriptHintLabel_->setWordWrap(true);

  scriptPathEdit_ = new LineEdit(this);
  scriptPathEdit_->setPlaceholderText(tr("/path/to/feed_script.py"));
  scriptHintLabel_->setVisible(false);
  scriptPathEdit_->setVisible(false);
  ...
  layout->addWidget(new QLabel(tr("Fetch type:")));
  layout->addWidget(fetchTypeCombo_);
  layout->addWidget(xpathHintLabel_);
  layout->addWidget(xpathRuleEdit_);
  layout->addWidget(scriptHintLabel_);
  layout->addWidget(scriptPathEdit_);
...
{
  int fetchType = fetchTypeCombo_->itemData(index).toInt();
  bool xpathMode = (fetchType == 1);
  bool scriptMode = (fetchType == 2);
  xpathHintLabel_->setVisible(xpathMode);
  xpathRuleEdit_->setVisible(xpathMode);
  scriptHintLabel_->setVisible(scriptMode);
  scriptPathEdit_->setVisible(scriptMode);
  resize(400, (xpathMode || scriptMode) ? 480 : 300);
}
...
void AddFeedWizard::addFeed()
{
  ...
  if (0 <= duplicateFoundId) {
    ...
    // Insert feed
    int fetchType = fetchTypeCombo_->itemData(fetchTypeCombo_->currentIndex()).toInt();
    QString fetchRule;
    if (fetchType == 1)
      fetchRule = xpathRuleEdit_->toPlainText().simplified();
    QString fetchScript;
    if (fetchType == 2)
      fetchScript = scriptPathEdit_->text().simplified();

    q.prepare("INSERT INTO feeds(xmlUrl, created, rowToParent, authentication, fetchType, fetchRule, fetchScript) "
              "VALUES (:feedUrl, :feedCreateTime, :rowToParent, :authentication, :fetchType, :fetchRule, :fetchScript)");
    ...
    q.bindValue(":fetchType", fetchType);
    q.bindValue(":fetchRule", fetchRule);
    q.bindValue(":fetchScript", fetchScript);
    q.exec();
                               ...
                               QDateTime dtReply, QString codecName)
{
  ...
  if (!data.isEmpty()) {
    // XPath / script mode: raw HTML goes straight to the parser; skip the
    // RSS feed detection logic.
