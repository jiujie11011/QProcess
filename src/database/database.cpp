...
const QString kCreateNewsTableQuery(
    ...
    // Version 18
    "scrollPos integer default 0, "        // last reading scroll offset
    "lastReadTime varchar, "               // last time the news was read
    "lastViewTime varchar, "               // last time the news was viewed
...
const QString kCreateStatsTableQuery(
...
const QString kCreateDialogTableQuery(
...
const QString kCreateExcludeAutoMarkTableQuery(
...
const QString kCreateRecommendationsTableQuery(
    "CREATE TABLE IF NOT EXISTS recommendations("
    "id integer primary key, "
    "newsId integer, "            // source article
    "date varchar, "
...
void Database::setPragma(QSqlDatabase &db)
{
  ...
  QString sync = settings.value("synchronousDB", "NORMAL").toString();
...
{
  {
    ...
    if (!db.open()) {
      ...
      if (!mainApp->dbFileExists()) {
        ...
        if (dbVersion < 18) {
          q.exec("ALTER TABLE news ADD COLUMN scrollPos integer default 0");
          q.exec("ALTER TABLE news ADD COLUMN lastReadTime varchar");
          q.exec("ALTER TABLE news ADD COLUMN lastViewTime varchar");
          q.exec("ALTER TABLE news ADD COLUMN aiSummary integer default 0");
          q.exec(kCreateStatsTableQuery);
          q.exec(kCreateDialogTableQuery);
          q.exec(kCreateExcludeAutoMarkTableQuery);
        }

        if (dbVersion < 19) {
          q.exec("ALTER TABLE news ADD COLUMN translatedTitle varchar");
          q.exec("ALTER TABLE news ADD COLUMN translatedContent varchar");
          q.exec("ALTER TABLE feeds ADD COLUMN fetchType integer default 0");
...
void Database::createTables(QSqlDatabase &db)
{
  ...
  // Version 18 tables
  db.exec(kCreateStatsTableQuery);
  db.exec(kCreateDialogTableQuery);
...
void Database::createIndexes(QSqlDatabase &db)
{
  db.transaction();

  db.exec("CREATE INDEX IF NOT EXISTS idx_news_feedId ON news(feedId)");
  db.exec("CREATE INDEX IF NOT EXISTS idx_news_guid ON news(guid)");
  db.exec("CREATE INDEX IF NOT EXISTS idx_news_deleted ON news(deleted)");
  db.exec("CREATE INDEX IF NOT EXISTS idx_news_feedId_deleted ON news(feedId, deleted)");
  db.exec("CREATE INDEX IF NOT EXISTS idx_news_feedId_read_deleted ON news(feedId, read, deleted)");
  db.exec("CREATE INDEX IF NOT EXISTS idx_news_title ON news(title COLLATE NOCASE)");
