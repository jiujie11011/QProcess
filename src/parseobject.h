...
private:
  ...
  int fetchType_;
  QString fetchRule_;
  QString fetchScript_;

  // Hash indexes for O(1) duplicate lookup (replaces linear scans over the
  // whole feed history). Each index maps a key field to the list of row
  // indices sharing that value; titleList_/publishedList_ give the value at a
  // given index.
  int existingNewsCount_;
  QHash<QString, QList<int> > guidIndex_;
  QHash<QString, QList<int> > linkIndex_;
  QHash<QString, QList<int> > titleIndex_;
  QHash<QString, QList<int> > publishedIndex_;
