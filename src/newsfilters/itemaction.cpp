  ...
  : QWidget(parent)
{
  ...
  itemList /*<< tr("Move News to")  << tr("Copy News to")*/
      ...
      << tr("Mark News as Unread") << tr("Move to Category")
      << tr("Save AI Summary");
  ...
  categoryEdit_->setPlaceholderText(tr("Category name"));
  ...
  buttonsLayout->addWidget(categoryWidget_, 1);
...
void ItemAction::currentIndexChanged(int index)
{
  ...
  if (index == 7) categoryWidget_->setVisible(true);
  else categoryWidget_->setVisible(false);
}
